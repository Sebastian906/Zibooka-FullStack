...
import { FaBook, FaListAlt, FaWarehouse, FaBrain, FaExclamationTriangle, FaBell } from 'react-icons/fa';
...
const Sidebar = () => {
    ...
    const navItems = [
        ...
        { path: "/admin/anomalies", label: t('admin.anomalies') || 'Anomalies', icon: <FaExclamationTriangle /> },
        { path: "/admin/notifications", label: t('admin.notifications'), icon: <FaBell /> }
    ]
                    ...
                    <div className='flex sm:flex-col sm:gap-x-5 gap-y-8 sm:pt-10'>
                        {navItems.map((link)=>(
                            <NavLink
                                ...
                                className={({ isActive }) => 
                                    isActive
                                        ? 'flexStart gap-x-2 n-5 lg:pl-12 medium-15 cursor-pointer h-10 text-secondary bg-white max-sm:border-b-4 sm:border-r-4 border-secondary'
