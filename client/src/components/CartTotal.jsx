    ...
    return (
        <div>
            <h3 className='bold-22'>
                {t('cart.orderDetails')} 
                <span className='bold-14 text-secondary'> ({getCartCount()}) {t('cart.items')}</span>
            </h3>
            <hr className='border-gray-300 my-5' />
            { /* PASARELA DE PAGOS */ }
            <div className='mb-5'>
                <div className='my-5'>
                    <h4 className='h4 mb-5'>{t('cart.whereToShip')}</h4>
                    <div className='relative flex justify-between items-start mt-2'>
                        <p>{selectedAddress ? `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state} ${selectedAddress.country}` : t('cart.noAddress')}</p>
                        <button
                ...
                <hr className='border-gray-300 my-5' />
                <div className='my-6'>
                    <h4 className='h4 mb-5'>{t('cart.paymentMethod')}</h4>
            ...
            <div className='mt-4 space-y-2'>
                <div className='flex justify-between'>
                    <h5 className='h5'>{t('cart.price')}</h5>
                ...
                <div className='flex justify-between'>
                    <h5 className='h5'>{t('cart.shippingFee')}</h5>
                    <p className='font-bold'>
                        {currency}
                ...
                <div className='flex justify-between'>
                    <h5 className='h5'>{t('cart.tax')} (2%)</h5>
                    <p className='font-bold'>
                        {currency}{(getCartAmount() * 2) / 100}
                    </p>
                </div>
                <div className='flex justify-between text-lg font-medium mt-3'>
                    <h5 className='h5'>{t('cart.totalAmount')}:</h5>
                    <p className='font-bold'>
                        {currency}
                        {getCartAmount() === 0 ? "$0.00" : getCartAmount() + delivery_charges + (getCartAmount() * 2) / 100}
                    </p>
                </div>
            </div>
            <button 
